...
class ControladorVentas{
	...
	static public function ctrCrearVenta(){
		...
		if(isset($_POST["nuevaVenta"])){
			...
			if($_POST["seleccionarComprobante"] === "COTIZACION"){
				...
				if($_POST["seleccionarTipoAnticipo"] == "A"){
					$valorEstado = "PROCESO";
				} else {
					$valorEstado = "PAGADO";
				}
				...
				$datos = array("id_vendedor"=>$_POST["idVendedor"],
							...
							"tipo_comprobante"=>$_POST["seleccionarComprobante"],
							"tipo_anticipo"=>$_POST["seleccionarTipoAnticipo"],
							"codigo"=>$_POST["nuevaVenta"],

							"productos"=>$_POST["listaProductos"],
							"impuesto"=>$_POST["nuevoImpuestoVenta"],
							"neto"=>$_POST["nuevoSubTotal"],
							"total"=>$_POST["nuevoTotalFinalVentaSA"],
							"prioridad" => "VENTA",
							"pagos" => $_POST["nuevaVenta"],
							"estado" => $valorEstado,
							"anticipo" => $_POST["nuevoAnticipo"],
	...
	static public function ctrPagarAnticipo(){

		
		if(isset($_POST["seleccionarBoleta"])){

			/*=============================================
			ACTUALIZAR LAS COMPRAS DEL CLIENTE Y REDUCIR EL STOCK Y AUMENTAR LAS VENTAS DE LOS PRODUCTOS
			=============================================*/

			if($_POST["listaProductosAnticipo"] == ""){
			...
			$serieBoleta = $_POST["seleccionarBoleta"];
			...
			$serieBoletaAnticipo = $venta["id"];

			$tipoComprobante = $venta["tipo_comprobante"];

			$vendedor = $venta["id_vendedor"];

			$impuestoAnticipo = $_POST["nuevoImporteAnticipo"]*0.18;

			$importeAnticipo = $_POST["nuevoImporteAnticipo"] - $impuestoAnticipo;
			...
			$datos = array("id_vendedor"=>$vendedor,
						   "id_cliente"=>$_POST["clienteAnticipo"],
						   "tipo_comprobante"=>$tipoComprobante,
						   "tipo_anticipo"=> "D.A.",
						   "codigo"=>$_POST["nuevoPagoAnticipo"],
						   "productos"=>$productos,
						   "impuesto"=>$impuestoAnticipo,
						   "neto"=>$importeAnticipo,
						   "total"=>$_POST["nuevoImporteAnticipo"],
						   "anticipo" => "0",
						   "prioridad" => "ANTICIPOVENTA",
						   "pagos" => $serieBoletaAnticipo,
						   "estado" => "PROCESO",
			...
			if( $_POST["deudaAnticipos"] == $_POST["nuevoImporteAnticipo"]){
			...
			$respuestaEstado = ModeloVentas::mdlActualizarVentas($tabla, $item1, $valor1, $item2, $valor2);
			
			
			

			if($respuesta == "ok" && $respuestaEstado == "ok"){

				echo'<script>

						Swal.fire({
						  title: "¡El Anticipo de venta ha sido guardada y actualizada correctamente",
						  icon: "success"
						}).then(function(result){
	...
	static public function ctrEditarVenta(){
		...
		if(isset($_POST["editarVenta"])){
			...
			if($cambioProducto){
				...
				$item1a = "total_compras";
				$valor1a = $traerCliente["total_compras"] - array_sum($totalProductosComprados);		
				...
				$item1a_2 = "total_compras";

				$valor1a_2 = array_sum($totalProductosComprados_2) + $traerCliente_2["total_compras"];
	...
	static public function ctrEliminarVenta(){
		...
		if(isset($_GET["idVenta"])){
			...
			if($traerVenta["tipo_comprobante"] !== 'COTIZACION'){
	...
	static public function ctrDescargarXML(){
		...
		if(isset($_GET["xml"])){
			...
			foreach ($listaProductos as $key => $value) {
				...
				$objetoXML->text($value["nombre_producto"].", ");
...
function mostrarSerieComprobante($idVenta) {
	...
	$traerVenta = ModeloVentas::mdlMostrarVentas($tabla, $item, $idVenta);

	$respuestaComprobante = "NO IDENTIFICADO";

	if($traerVenta["tipo_comprobante"] == "BOLETA"){
		$respuestaComprobante = "B001";
	} else if($traerVenta["tipo_comprobante"] == "FACTURA") {
		$respuestaComprobante = "F001";
	} else if($traerVenta["tipo_comprobante"] == "COTIZACIÓN"){
		$respuestaComprobante = "C001";
	} else{
		$respuestaComprobante = "N.V.001";
